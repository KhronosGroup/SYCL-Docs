...
same attribute applied (with its same parameters), but this is not required.
The attribute remains in effect regardless of whether it appears in the
redeclaration.

Programs which apply the same attribute (with either the same or different
parameters) more than once to the same kernel or device function declaration
are ill formed.  The compiler must issue a diagnostic for programs which
...
attribute (and its same parameters) in another translation unit, the program is
