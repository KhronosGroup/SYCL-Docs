...
====
  ...
  * Any variable or function that is odr-used from a <<device-function>> must be
    ...
    <<subsec:syclexternal>>.
  * Use of [code]#long double# types results in undefined behavior.
...
However, the availability of these types in device code does not guarantee that
they will be supported by all devices.
