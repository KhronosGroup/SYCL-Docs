...
SYCL device compilers should give a compilation error if the required
work-group size is unsupported.  If the kernel is submitted for execution using
an incompatible work-group size, the SYCL runtime must throw an
...
SYCL device compilers should give a compilation error if the required sub-group
size is unsupported by the device or incompatible with any language feature used
by the kernel.  The set of valid sub-group sizes for a kernel can be queried as
described in <<table.device.info>> and
