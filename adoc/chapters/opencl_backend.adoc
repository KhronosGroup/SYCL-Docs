...
For OpenCL <<backend>> this detail is abstracted away by
<<kernel-bundle,kernel bundles>> and a [code]#kernel_bundle# object
containing all <<sycl-kernel-function,SYCL kernel functions>> is retrieved
by calling the free function [code]#get_kernel_bundle#.
