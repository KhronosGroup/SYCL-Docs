...
These constructors must retain that instance to increase the reference count
of the OpenCL resource.

The destructor for the <<sycl-runtime>> classes which encapsulate an
OpenCL opaque type must release that instance to decrease the reference
count of the OpenCL resource.
...
----
   a@ [code]#cl_device_id#
   a@ [code]#cl_device_id#
   a@ A SYCL device object encapsulates an OpenCL device object.
a@
[source]
----
context
...
All free functions are available in the [code]#sycl::opencl# namespace.
...
sycl::buffer<T, dimensions, AllocatorT> make_buffer(
...
All OpenCL objects are reference counted. The SYCL general programming model
