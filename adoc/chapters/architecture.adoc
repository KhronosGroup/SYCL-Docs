...
26 to 29.  The <<command-group-scope>> specifies a unit of work which is
comprised of a <<sycl-kernel-function>> and <<accessor,accessors>>.  In this
example <<command-group-scope>> is defined by lines 21 to 30.  The
...
of a <<command-group-function-object>> as seen on line 21. The
...
for [code]#myQueue# on line 15 is the default constructor, which allows
...
<<sec:memory.model>>. We
construct a buffer on line 18. Access to the <<buffer>> is controlled via
an <<accessor>> which is constructed on line 23.
...
write to the data buffer on line 28.
