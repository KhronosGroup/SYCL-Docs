...
[[byval-semantics]]SYCL common by-value semantics::
    Several <<sycl-runtime>> classes have semantics comparable to those required
    by the {cpp} [code]#PODType# and [code]#TrivialType# named requirements.
    These semantics are collectively known as the _SYCL common by-value
    semantics_.
    See <<sec:byval-semantics>> for details.

[[reference-semantics]]SYCL common reference semantics::
    Several <<sycl-runtime>> classes have semantics comparable to those of
    `std::shared_ptr`, where multiple instances of a class share ownership of an
    implementation-defined resource, such as a <<native-backend-object>>.
    These semantics are collectively known as the _SYCL common reference
    semantics_.
    See <<sec:reference-semantics>> for details.
