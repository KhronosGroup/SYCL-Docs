...
In host code, the [code]#get_native# function must throw a synchronous
[code]#exception# with the [code]#errc::backend_mismatch# error code if the
backend of the SYCL object doesn't match the target backend.
In device code, the behavior is undefined if the backend of the SYCL object
