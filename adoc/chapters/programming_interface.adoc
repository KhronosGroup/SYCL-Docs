...
When the user wants to use a [code]#buffer# simultaneously in the
<<sycl-runtime>> (e.g. in a command running on a device) and their own code (e.g. a multi-threaded mechanism) and wants
...
When the runtime releases the mutex, the user is guaranteed that the data has been
copied back through the shared pointer --- unless the final data destination has been
