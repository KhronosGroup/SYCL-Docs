...
----
    ...
    @ [code]#range<3>#
   a@ Returns the maximum global work size.  This descriptor is only valid if
      the device type is [code]#device_type::custom# or if the kernel is a
      built-in kernel, and the exact semantics are defined by each SYCL backend
      specification.
