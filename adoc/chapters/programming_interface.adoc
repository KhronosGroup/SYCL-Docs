...
* If the specification of a function has several occurrences of the same
  generic type name, replace each occurrence in the same overload with the same
  actual type.  For example, the [code]#fmax# example above has three
  occurrences of [code]#genfloat#: one for the return type and one for each
  parameter type.  The available overloads include only the cases where both
  parameters have the same type, and the return type always matches the type of
  the parameters.
...
* Some of the generic type names are replaced with a templated type, where the
  identifiers [code]#N#, [code]#Space#, and [code]#IsDecorated# represent
...
/*return-type*/ fmax(NonScalar x, typename NonScalar::value_type y)
----
!====
...
template<typename NonScalar>                                         (5)
/*return-type*/ fmin(NonScalar x, typename NonScalar::value_type y)
----
!====
...
![code]#vec<uint32_t, N># +
 [code]#+__swizzled_vec__<uint32_t, N>+#
![code]#vec<float, N>#

![code]#vec<uint64_t, N># +
 [code]#+__swizzled_vec__<uint64_t, N>+#
![code]#vec<double, N>#

![code]#vec<uint16_t, N># +
 [code]#+__swizzled_vec__<uint16_t, N>+#
![code]#vec<half, N>#
!====
...
* [code]#+__swizzled_vec__<uint8_t, N>+#
* [code]#+__swizzled_vec__<uint16_t, N>+#
* [code]#+__swizzled_vec__<uint32_t, N>+#
* [code]#+__swizzled_vec__<uint64_t, N>+#
...
template<typename NonScalar>                                               (2)
/*return-type*/ clamp(NonScalar x, typename NonScalar::value_type minval,
...
template<typename NonScalar>                                        (2)
/*return-type*/ max(NonScalar x, typename NonScalar::value_type y)
----
!====
...
template<typename NonScalar>                                        (2)
/*return-type*/ min(NonScalar x, typename NonScalar::value_type y)
----
!====
...
* [code]#vec<double, N>#
* [code]#vec<half, N>#
* [code]#+__swizzled_vec__<float, N>+#
* [code]#+__swizzled_vec__<double, N>+#
* [code]#+__swizzled_vec__<half, N>+#
...
template<typename NonScalar1, typename NonScalar2>                                  (2)
/*return-type*/ mix(NonScalar1 x, NonScalar2 y, typename NonScalar1::value_type a)
----
!====
...
template<typename NonScalar>                                            (2)
/*return-type*/ step(typename NonScalar::value_type edge, NonScalar x)
----
!====
...
[source]
----
typename GenFloat1::value_type t;
...
/*return-type*/ smoothstep(
  typename NonScalar::value_type edge0,
  typename NonScalar::value_type edge1,
  NonScalar x)
----
!====
...
[source]
----
typename NonScalar::value_type t;
...
** [code]#vec<T, N>#, where [code]#T# is one of the _vector element types_ as
   defined above; or
** [code]#+__swizzled_vec__<T, N>+#, where [code]#T# is one of the _vector
   element types_;
