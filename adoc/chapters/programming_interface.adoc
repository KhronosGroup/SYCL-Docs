...
code can concurrently access the same memory.  Device code and host code can
perform fence and atomic operations on objects in this memory, and those
operations are guaranteed to interoperate in the following ways when the device
code uses memory scope [code]#memory_scope::system#.
