...
parameters in [code]#info::platform#. Every <<platform>>
must produce a valid value for each info parameter. The possible values for
each info parameter and any restrictions are defined in the specification
of the <<backend>> associated with the <<platform>>. All info parameters in
...
parameters in [code]#info::context#. Every <<context>>
must produce a valid value for each info parameter. The possible values for
each info parameter and any restrictions are defined in the specification of
the <<backend>> associated with the <<context>>. All info parameters in
...
parameters in [code]#info::device#. Every <<device>> must produce a
valid value for each info parameter. The possible values for each info
...
parameters in [code]#info::queue#. Every <<queue>>
must produce a valid value for each info parameter. The possible values for
each info parameter and any restriction are defined in the specification of
the <<backend>> associated with the <<queue>>. All info parameters in
[code]#info::queue# are specified in <<table.queue.info>> and the synopsis
for [code]#info::queue# is described in <<appendix.queue.descriptors>>.
...
parameters in [code]#info::event#. Every <<event>>
must produce a valid value for each info parameter. The possible values for
each info parameter and any restrictions are defined in the specification
of the <<backend>> associated with the <<event>>. All info parameters in
[code]#info::event# are specified in <<table.event.info>> and the synopsis
for [code]#info::event# is described in <<appendix.event.descriptors>>.
...
[code]#info::event_profiling#. Every <<event>> must produce a valid value
for each info parameter. The possible values for each info parameter and
...
----
   a@ This function must only be used to invoke a kernel that was constructed
      ...
      results in undefined behavior.  The precise semantics of this function
      are defined by each SYCL backend specification, but the intent is that
      the kernel should execute exactly once.
...
----
   a@ This function must only be used to invoke a kernel that was constructed
      ...
      results in undefined behavior.  The precise semantics of this function
      are defined by each SYCL backend specification, but the intent is that
      the kernel should be invoked for the specified range of index values.
...
----
   a@ This function must only be used to invoke a kernel that was constructed
      ...
      results in undefined behavior.  The precise semantics of this function
      are defined by each SYCL backend specification, but the intent is that
      the kernel should be invoked for the specified [code]#ndrange#.
...
The bundle's set of associated devices is [code]#devs#.
...
kernels with the given identifiers.  These identifiers may represent kernels
that are defined in the application, device built-in kernels, or a mixture of
the two.  Since the device images may group many kernels together, the returned
bundle may contain additional kernels beyond those that are requested in
[code]#kernelIds#.  The bundle's set of associated devices is [code]#devs#.
...
_Throws:_
  ...
  * An [code]#exception# with the [code]#errc::invalid# error code if any of
    the kernels are incompatible with all devices in [code]#devs#.
  * An [code]#exception# with the [code]#errc::invalid# error code if any of
    the devices in [code]#devs# is not in the list of devices associated with
    the context [code]#ctxt#.
  * An [code]#exception# with the [code]#errc::invalid# error code if
...
application is compatible with a device unless it uses optional features which
are not supported on the device, as described in
<<sec:optional-kernel-features>>.  A device built-in kernel is only compatible
with the device for which it is built-in.
...
images in the input [code]#bundles# with duplicates removed.
...
new bundle's set of associated devices is [code]#devs#.
...
_Throws:_
  ...
  * An [code]#exception# with the [code]#errc::invalid# error code if any of
    ...
    [code]#kernel_bundle::get_devices()#).
  * An [code]#exception# with the [code]#errc::build# error code if the online
...
Every kernel must produce a valid value for each info parameter.  All info
parameters in [code]#info::kernel# are specified in <<table.kernel.info>> and
the synopsis for [code]#info::kernel# is described in
...
----
    ...
    @ [code]#uint32_t#
   a@ This descriptor is only meaningful when used to query a kernel that
      resides in a kernel bundle that was constructed using a backend specific
      interoperability function.  Attempting to use this query for other
      kernels returns an unspecified value.  The semantics of this descriptor
...
[code]#get_info()# member function, specifying one of the info
parameters in [code]#info::kernel_device_specific#.  Every kernel must produce
a valid value for each info parameter.  All info parameters in
...
----
    ...
    @ [code]#range<3>#
   a@ Returns the maximum global work size.  Only valid if device type is
      [code]#device_type::custom# or the kernel is a built-in kernel.
