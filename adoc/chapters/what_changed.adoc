...
can be used to block work-items in either <<work-group, work-groups>> or
<<sub-group, sub-groups>> until all work-items in the group arrive at the
barrier. The [code]#mem_fence# member function has been replaced by the
