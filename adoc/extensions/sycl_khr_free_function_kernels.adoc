...
that is decorated with either the [code]#SYCL_KHR_ND_KERNEL# or
the [code]#SYCL_KHR_TASK_KERNEL# macro so that it becomes a device kernel entry
point.
...
with either the [code]#SYCL_KHR_ND_KERNEL# or the [code]#SYCL_KHR_TASK_KERNEL#
macro.
...
[code]#Func# is decorated as a free function kernel of a particular kind (see
<<sec:khr-free-function-kernels-defining>>).
