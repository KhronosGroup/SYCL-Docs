...
_Effects (4-6)_: Equivalent to [code]#q.submit([&](handler& h) { launch(h, r, k,
args...); })#.
...
_Effects (2)_: Equivalent to [code]#q.submit([&](handler& h) { launch_task(h, k,
args...); })#.
...
* All commands associated with this command's dependencies (e.g., via
  `handler::depends_on`) have completed.
...
other dependencies (e.g., specified by [code]#handler::depends_on#), it is not
