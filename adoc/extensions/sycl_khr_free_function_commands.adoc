...
* [code]#signal_event#: When the command is complete, signals the provided
  event.
...
this operation may be a no-op.
If an event wrapped in signal_event was provided in [code]#reqs#, it will be
signaled.{endnote}
...
* All commands previously submitted to this queue have completed; and
* All events (wrapped in wait_event or wait_events) in [code]#reqs# have
  completed.

Once the above conditions are met, signals the provided event ([code]#e#).

{note}The event is disassociated from any previous command it was
tracking.{endnote}
...
Once the above conditions are met, signals the event (wrapped in signal_event),
if provided.
...
----
_Effects_: Creates an event associated with a given context.
The event can then be used with [code]#enqueue_signal_event# function, with
signal_event object and as a dependency for other commands.
