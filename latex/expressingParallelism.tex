...
\addFootNotes{Member functions of the SYCL \codeinline{program} class}{table.members.program}
  ...
  \addRow
    ...
    {
      Links the encapsulated \codeinline{cl_program} with the compiler options specified by \codeinline{linkOptions}, if this SYCL \codeinline{program} is an OpenCL program. Sets the state of this SYCL \codeinline{program} to \codeinline{program_state::linked}. Must throw an \codeinline{invalid_object_error} SYCL exception if this \codeinline{program} was not in the \codeinline{program_state::compiled} state when called. Must return a  \codeinline{compile_program_error} SYCL exception if the linking fails.
    }
  \addRowTwoL
    {template <typename kernelT>}
